...
class TextDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = row["score"]
        if self.labelintext:
            comment = f"{100*round(label)}: {comment}"
...
class TextCollator:
    ...
    def __call__(self, raw_batch: Sequence[Tuple[str, torch.Tensor]]) -> \
        ...
        #print("SHAPE TOKEN : ", np.asarray(tokens).shape)
        #print("SHAPE TOKEN : ", np.asarray(targets).shape)
...
def make_dataloaders(config: ConfigFile):
    ...
    valid_data = pd.read_csv("data/train.csv", index_col=0)
    test_data = pd.read_csv("data/train.csv", index_col=0)
...
if __name__ == "__main__":
    ...
    for text, tokens,score,target in train_dl:
