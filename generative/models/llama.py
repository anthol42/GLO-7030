...
class Transformer(nn.Module):
    def __init__(self, params: ModelArgs, labelintext = False):
        super().__init__()
        self.params = params
        ...
        #if not labelintext:
        #self.tox_projector = nn.Linear(1,params.dim)
    ...
    def forward(self, idx, tox = None, start_pos:int =-1):
            B, T = idx.size()
            #assert T <= self.config["block_size"], f"Cannot forward a text {T} bigger than blocksize {self.config['block_size']}"
            mask = None # torch.cat((torch.zeros((B, 1), device=idx.device), idx == -1), dim=1)   # -1 is Padding idx
            # To avoid out of bounds error in embeddings
            self.labelintext = True
            ...
            if T > 1:
                ...
                mask = torch.hstack(
                    [torch.zeros((T, stsart_pos), device=idx.device), mask]
                ).type_as(x)
            """
            idx[idx == -1] = 0
            if self.labelintext:
                ...
                tok_emb = self.tok_embeddings(idx)  # Shape(B, T, n_emb)
                tox_emb = self.tox_projector(tox.view(B, 1, 1)) # IN: Shape(B, 1) OUT: (B, 1, n_emb)
                x = tok_emb + tox_emb    # Implicit broadcasting Shape(B, T, n_emb)
                #x = tok_emb
    ...
    def generate(
        ...
        """
            ...
            tokens = torch.full((bsz, total_len), pad_id, dtype=torch.long, ).to(device)
            for k, t in enumerate(prompt_tokens):
                tokens[k, : len(t)] = torch.tensor(t, dtype=torch.long, ).to(device)

            prev_pos = 0
            eos_reached = torch.tensor([False] * bsz, ).to(device)
            input_text_mask = tokens != pad_id

            if min_prompt_len == total_len:
                logits = self.forward_inference(tokens, prev_pos)

            stop_tokens = torch.tensor(list(tokenizer.stop_tokens)).to(device)
            ...
            for cur_pos in range(min_prompt_len, total_len):
                ...
                if temperature > 0:
                    ...
                    next_token = sample_top_p(probs, top_p, sample_rng).to(device)
                else:
                    next_token = torch.argmax(logits[:, -1], dim=-1)
                next_token = next_token.reshape(-1).to(device)
    ...
    def text_completion(
        ...
        completions = [{"generation": tokenizer.decode(t)} for t in generation_tokens]
        return completions

            

    def forward_loss(self, inputs: torch.Tensor, targets: torch.Tensor, ignore_index=-100,tox = 0.5):
    ...
    def freeze(self, transformer: int = 0, embeddings: bool = False , ln_f: bool = False):
        num_layer_to_freeze = int(len(self.layers)  - transformer)
        print("NLAYER TO FREEZE : ", num_layer_to_freeze)
...
class Llama:
    ...
    def __init__(self, model: Transformer, tokenizer: Tokenizer,config):
        self.model = model
...
if __name__ == "__main__":
    ...
    prompts = ["Toxicity 80.0;"]

    print(model.text_completion(tokenizer,device,prompts,sample_rng,max_gen_len=256))
